...
1. **Every label the SDL declares reaches an element.** Each `@node` label is a vertex element and each `@relationship` label is an edge element on the table it names, checked by re-deriving the set from the SDL rather than from the code that built the model. This is the one invariant that exists to catch the *class* rather than a case (gopgql#49): everything gopgql generates from an SDL — the graph DDL, a compiled `GRAPH_TABLE`, a typed client — names elements by label, so a label the model quietly lacks is not an error until PostgreSQL rejects the `MATCH` in the consumer's process. Two commands cannot check each other this way, though: `--graph` names the graph and neither `generate` nor `generate client` can see the other's output, so a mismatched graph name stays a run-time failure and `conform` is the tool for it.
...
- **A generated decoder is handed `shape`'s canonical form, never the driver's value.** By the time a generated assignment runs, `shape.Rows` or `shape.Decode` has already turned every number into a `json.Number` of its digits, every timestamp into RFC 3339 text in UTC, and every uuid into 8-4-4-4-12 text — that canonicalisation is what M8 rests on. A generated *mutation* is the exception and the reason each decoder has two arms: `exec.Call` shapes nothing, so a `@function`'s scalar arrives exactly as pgx scanned it. Two producers, one decoder, agreeing by convention rather than through a shared type — this is the only such seam in gopgql.
...
**Where the runtime comes from.** `docs/package.json` depends on an npm package tarball published as a **release asset of `gaarutyunov/postgres-pglite`**, by URL:
