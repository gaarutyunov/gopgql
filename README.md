...
[Releases page](https://github.com/gaarutyunov/gopgql/releases) — one `tar.gz`
...
```sh
go install github.com/gaarutyunov/gopgql/cmd/gopgql@latest
go install github.com/gaarutyunov/gopgql/cmd/gopgql-mcp@latest
```

Or as a container image, published to GHCR on every release:

```sh
docker run --rm ghcr.io/gaarutyunov/gopgql:latest gopgql version
```
...
services:
  ...
  migrate:
    image: ghcr.io/gaarutyunov/gopgql:latest
    depends_on:
...
[goga](https://github.com/gaarutyunov/goga)'s `telemetry` module: traces and
